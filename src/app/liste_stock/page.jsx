...
import { getOperationsByDelegations, getCurrentUser } from "/src/api";
...
const Page = () => {
  ...
  const { data: operations, refetch: refetchOperations } = useQuery({
    ...
    queryFn: () => getOperationsByDelegations(userData?.unite?.delegationId),
  ...
  const formatDate = (dateArray) => {
    if (!Array.isArray(dateArray) || dateArray.length < 6) {
      return "Date invalide"; // Retourner une valeur par défaut ou un message
    }
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      ...
      <div className="flex-grow p-6">
        <h2 className="text-2xl font-bold mb-4">Mes Opérations</h2>
                ...
                {currentOperations.map((operation) => (
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {formatDate(operation.dateCreation)}
                    </td>
