...
export function DataTable({
  ...
  const pageSize = 10; // Taille de page à ajuster selon vos besoins
  ...
  const table = useReactTable({
    ...
    pageCount: Math.ceil(data.length / pageSize), // Calculer le nombre de pages en fonction des données et de la taille de page
    manualPagination: true, // Activer la pagination manuelle
    onSortingChange: setSorting,
    onColumnFiltersChange: setColumnFilters,
    onPaginationChange: (updater) => {
      const newPageIndex = typeof updater === 'function' ? updater(pageIndex) : updater.pageIndex;
      setPageIndex(newPageIndex); // Mise à jour de l'état de pagination
    },
